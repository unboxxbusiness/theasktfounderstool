...
const getDefaultFounders = (searchParams: URLSearchParams | null) => {
    if (!searchParams) return [{ name: 'Founder 1', idea: 50, time: 50, money: 50 }];
...
export function EquitySplitCalculator() {
  ...
  const form = useForm<FounderFormState>({
    ...
    defaultValues: {
      founders: getDefaultFounders(null),
    },
  });
  ...
  useEffect(() => {
    if(searchParams) {
        ...
        form.reset({ founders: getDefaultFounders(searchParams) });
    }
  }, [searchParams, form]);
