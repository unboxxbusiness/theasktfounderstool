...
export function CacLtvCalculator() {
  ...
  useEffect(() => {
    setMarketingSpend(Number(searchParams.get('marketingSpend')) || 10000);
    setCustomersAcquired(Number(searchParams.get('customersAcquired')) || 500);
    setAvgMonthlySpend(Number(searchParams.get('avgMonthlySpend')) || 50);
    setGrossMargin(Number(searchParams.get('grossMargin')) || 80);
    setMonthlyChurn(Number(searchParams.get('monthlyChurn')) || 5);
  }, [searchParams]);
