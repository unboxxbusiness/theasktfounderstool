...
export function MarketingBudgetAllocator() {
  const searchParams = useSearchParams();
  const [totalBudget, setTotalBudget] = useState(50000);
  const [goal, setGoal] = useState<MarketingGoal>('leads');
  const [shareUrl, setShareUrl] = useState('');

  useEffect(() => {
    setTotalBudget(Number(searchParams.get('totalBudget')) || 50000);
    setGoal((searchParams.get('goal') as MarketingGoal) || 'leads');
  }, [searchParams]);
